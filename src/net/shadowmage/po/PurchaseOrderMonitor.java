...
public class PurchaseOrderMonitor
{
  ...
  public static void main(String[] args)
  {
    ...
    try
    {
      input = new FileInputStream("config/config.cfg");
      prop.load(input);
      PurchaseOrderMonitor monitor = new PurchaseOrderMonitor(prop);
      monitor.runMonitor();
    }
    catch (Exception e)
    {
   ...
   */
  private PurchaseOrderMonitor(Properties configFile)
  {
    ...
    System.out.println("Setting up Directory Monitor for Purchase Order processing:");
  ...
  private void monitorLoop()
  {
    ...
    for(File fileToParse : files)
    {
      ...
      try
      {
        ...
        PurchaseOrderParser.parsePOs(config, fileToParse, localTempDirectory, localOutputDirectory);
      }
      catch (IOException e)
      {
        e.printStackTrace();
